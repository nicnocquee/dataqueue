...
Progress is stored in the `progress` field of the [JobRecord](/docs/api/job-record):
...
If you're using the [React SDK](/docs/usage/react-sdk), the `useJob` hook exposes `progress` directly:
...
<Callout>
  ...
  Migration](/docs/usage/database-migration).
</Callout>
