...
<Callout>
  Database migrations are **only required for the PostgreSQL backend**. If
...
<Callout>
  **Matching schema at runtime:** If you run migrations with `--schema
  dataqueue`, your app must use the same schema when connecting. Include it in
  the connection string with `?search_path=dataqueue` or `?schema=dataqueue`.
  DataQueue supports both parameters; use the same value you passed to
  `--schema` so the queue finds the tables.
</Callout>
