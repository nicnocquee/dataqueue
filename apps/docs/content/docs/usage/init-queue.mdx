...
<Callout>
  ...
  To use a custom schema, add either `?search_path=my_schema` (Postgres convention)
  or `?schema=my_schema` (e.g. Prisma-style). If both are present, `search_path`
  takes precedence.

```dotenv
# Using search_path (Postgres convention)
PG_DATAQUEUE_DATABASE=postgresql://postgres:password@localhost:5432/my_database?search_path=my_schema

# Or using schema (e.g. Prisma-style)
PG_DATAQUEUE_DATABASE=postgresql://postgres:password@localhost:5432/my_database?schema=my_schema
```
...
<Callout>
  **Connection ownership:** When you provide your own `pool` or `client`, the
