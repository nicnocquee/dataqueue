...
If you use a custom schema (e.g. `--schema dataqueue`), include the same schema in your connection string with `?search_path=dataqueue` or `?schema=dataqueue` so the app finds the tables.
